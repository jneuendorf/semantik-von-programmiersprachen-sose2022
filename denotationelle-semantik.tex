...
\begin{example}
    ...
    \begin{itemize}
        \item definition-use-Analyse: Sind Variablen initizialisiert bevor sie benutzt werden?
...
\begin{enumerate}
    \item Wir definieren die gewünschte Eigenschaft, \dh{} eine Variable ist entweder POS, NEG oder NULL.
        ...
        Füge zusätzliche Eigenschaften hinzu, um Analuse detaillierter zu machen und einen vollständigen Verband zu erhalten: NONPOS, NONNEG, NONNULL, BEL und NONE (letzteres aus technischen Gründen: um für die leere Menge ein Supremum zu haben).
        ...
        \begin{figure}[H]
            ...
            \end{tikzcd}
            \caption{Hasse-Diagramm für $(\mathcal{P}, \sqsubseteq_{\mathcal{P}})$ (nach oben allgemeiner, nach unten spezifischer)}
        \end{figure}
    ...
    \item Definiere \emph{Analysefunktion}: Analog zur semantischen Funktion, operiert aber auf Zustandseigenschaften anstelle von Zuständen: Für alle syntaktischen Kategorien definiere:
        ...
        \begin{enumerate}
            \item Arithmetische Ausdrücke \begin{align*}
                    ...
                    \mathcal{DA}\lsem z \rsem(\tau) & = \abs_{\mathbb{Z}}(\mathcal{N}(z)) \\
                    \mathcal{DA}\lsem x \rsem(\tau) & = \tau(z) \\
                    \mathcal{DA}\lsem a_1 \square a_2 \rsem(\tau) & = \mathcal{DA}\lsem a_1 \rsem(\tau) \square_v \mathcal{DA}\lsem a_2 \rsem(\tau) \tag{*} \\
                \end{align*}
            ...
            \item Boolesche Ausdrücke \begin{align*}
                    \mathcal{DB} : \BExp \to (\PState & \to \mathcal{P}) \\
                    \mathcal{DB}\lsem\texttt{true}\rsem(\tau) & = \true \\
                    \mathcal{DB}\lsem\texttt{false}\rsem(\tau) & = \false \\
                    \mathcal{DB}\lsem a_1 \square a_2 \rsem(\tau) & = \mathcal{DA}\lsem a_1 \rsem(\tau) \square_v \mathcal{DA}\lsem a_2 \rsem(\tau) \\
                    \mathcal{DB}\lsem \neg b \rsem(\tau) & = \neg_{\tau} \mathcal{DB}\lsem b \rsem(\tau) \\
                    \mathcal{DB}\lsem b_1 \square b_2 \rsem(\tau) & = \mathcal{DB}\lsem b_1 \rsem(\tau) \square_{\tau} \mathcal{DB}\lsem b_2 \rsem(\tau)
                \end{align*}

            \item Anweisungen \begin{align*}
                    \mathcal{DS} : \Stm \to (\PState & \to \mathcal{P}) \\
                    \mathcal{DS}\lsem x \texttt{ := } a \rsem(\tau) & = \sigma[x \mapsto \mathcal{DA}\lsem a \rsem(\sigma)] \\
                    \mathcal{DS}\lsem \texttt{skip} \rsem(\tau) & = \sigma \\
                    \mathcal{DS}\lsem S_1 \texttt{; } S_2 \rsem(\tau) & = \mathcal{DS}\lsem S_2 \rsem(\tau) \circ \mathcal{DS}\lsem S_1 \rsem(\tau) \\
                    \mathcal{DS}\lsem \texttt{if $b$ then $S_1$ else $S_2$} \rsem(\tau) & = \cond_{\infty}(\mathcal{DB}\lsem b \rsem, \mathcal{DS}\lsem S_1 \rsem, \mathcal{DS}\lsem S_2 \rsem) \\
                    \mathcal{DS}\lsem \texttt{while $b$ do $S$} \rsem(\tau) & = \fix(H) \\
                    H: h & \mapsto \cond_{\infty}(\mathcal{DB}(b), h \circ \mathcal{DS}(S), \id)
                \end{align*}

                mit \begin{align*}
                    \cond_{\infty}(f, h_1, h_2)(\sigma) & = \begin{cases}
                        \text{INIT} & \text{falls } f(\tau) = \text{NONE} \\
                        h_1(\tau) & \text{falls } f(\tau) = \true \\
                        h_2(\tau) & \text{falls } f(\tau) = \false \\
                        \sup \{ h_1(\tau), h_2(\tau) \} & \text{falls } f(\tau) = \text{BEL} \\
                    \end{cases}
                \end{align*}
